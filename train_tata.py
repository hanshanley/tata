...
class StanceClassifier(torch.nn.Module):
    '''
    This module performs stance classification using deBERTa embeddings.
    '''
    def __init__(self, config):
        super(StanceClassifier, self).__init__()
        self.tag = AutoModel.from_pretrained('microsoft/deberta-v3-base',cache_dir = 'cache')
        self.tag.load_state_dict(torch.load('tag_embeddings.pt'), strict=False)
        self.taw =AutoModel.from_pretrained('microsoft/deberta-v3-base',cache_dir = 'cache')
        self.taw.load_state_dict(torch.load('taw_embeddings.pt'), strict=False)
        self.deberta = AutoModel.from_pretrained('microsoft/deberta-v3-base',cache_dir = 'cache')
        
        
        for param in self.taw.parameters():
            param.requires_grad = False
        for param in self.tag.parameters():
            param.requires_grad = False
        ...
        self.linear_taw = torch.nn.Linear(HIDDEN_SIZE,HIDDEN_SIZE)
        self.linear_tag = torch.nn.Linear(HIDDEN_SIZE,HIDDEN_SIZE)
        
        self.dropout_21 = torch.nn.Dropout(config.hidden_dropout_prob)
        self.linear_21 = torch.nn.Linear(2*HIDDEN_SIZE,HIDDEN_SIZE)
        self.linear_21.requires_grad = True
        self.relu_21 = torch.nn.ReLU()
        
        
        self.dropout_11 = torch.nn.Dropout(config.hidden_dropout_prob)
        self.linear_11 = torch.nn.Linear(2*HIDDEN_SIZE,HIDDEN_SIZE)
        self.linear_11.requires_grad = True
        self.relu_11 = torch.nn.ReLU()

        self.total_linear = torch.nn.Linear(2*HIDDEN_SIZE,HIDDEN_SIZE)
    ...
    def predict_stance(self,input_ids_1, attention_mask_1,
        ...
        tag_embeddings = self.tag(input_ids_3, attention_mask_3)['last_hidden_state'][:,0,:]
        tag_text_aware_attention = self.attention(text_token_emebddings.unsqueeze(1),self.linear_tag(tag_embeddings))

        taw_embeddings = self.taw(input_ids_3, attention_mask_3)['last_hidden_state'][:,0,:]
        topic_taw_embeddings = self.attention(taw_embeddings.unsqueeze(1),self.linear_taw(topic_token_emebddings))
        
        
        output1 = torch.cat((tag_text_aware_attention,text_token_emebddings), axis = -1)
        output1 = self.dropout_11(output1)
        output1 = self.linear_11(output1)
        output1  = self.relu_11(output1)
        
        output2 = torch.cat((topic_taw_embeddings,text_token_emebddings), axis = -1)
        output2 = self.dropout_21(output2)
        output2 = self.linear_21(output2)
        output2  = self.relu_21(output2)
        
        total_output = torch.cat((output1,output2), axis = -1)
        total_output =self.total_linear(total_output)
        total_output = self.dropout_2(total_output)
        total_output = self.linear_2(total_output)
        total_output  = self.relu_2(total_output)
        total_output = self.batchnorm_2(total_output)
        total_output = self.linear_3(total_output) 
        return total_output
